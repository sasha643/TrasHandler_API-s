...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        def handle_message(message):
            ...
            # Fetch all accepted pickup requests for the vendor
            pickup_requests = PickupRequest.objects.filter(vendor=vendor, status='Accepted')

            if not pickup_requests.exists():
                self.stdout.write(f"No assigned pickup request found for vendor {vendor_id}")
                return

            # Process each pickup request individually
