...
This signal is triggered whenever a `Notification` instance is saved. It is responsible for sending the notification asynchronously and marking the notification as read if it is relevant and not related to reassignment.

**Actions:**

- Sends the notification using the `send_notification_task` Celery task.
- Marks the notification as read/sent if it is relevant, meaning it is accepted and not related to reassignment.
